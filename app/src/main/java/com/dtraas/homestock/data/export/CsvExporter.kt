...
/**
 * Builds a CSV export of Voorraad/Boodschappenlijst — plain, Compose-independent functions so
 * they're easy to call from a click handler (not a composable context) and to unit test. Column
 * headers and the "Ja"/"Nee" values are passed in already localized (built once via
 * `stringResource` in MoreScreen, where the export is triggered from) rather than hardcoded here,
 * so the exported file matches whatever language the app itself is currently in — same as every
 * other piece of user-facing text in the app.
 *
