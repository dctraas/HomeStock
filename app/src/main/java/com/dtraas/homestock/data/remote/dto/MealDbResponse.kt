package com.dtraas.homestock.data.remote.dto
...
data class MealDbLookupResponse(
    @SerializedName("meals") val meals: List<MealDbDetail>?,
)

/**
 * TheMealDB has no ingredients array — each recipe is a flat row with up to 20
 * numbered ingredient/measure column pairs, most of them blank. [ingredients]
 * zips the non-blank ones together in order.
 */
data class MealDbDetail(
    @SerializedName("idMeal") val id: String,
    @SerializedName("strMeal") val name: String,
    @SerializedName("strMealThumb") val thumbnailUrl: String?,
    ...
    /** Ingredient name paired with its measure (e.g. "Chicken breast" to "200g"), blanks dropped. */
    val ingredients: List<Pair<String, String>>
        get() = listOf(
            ingredient1 to measure1, ingredient2 to measure2, ingredient3 to measure3, ingredient4 to measure4,
            ingredient5 to measure5, ingredient6 to measure6, ingredient7 to measure7, ingredient8 to measure8,
            ingredient9 to measure9, ingredient10 to measure10, ingredient11 to measure11, ingredient12 to measure12,
            ingredient13 to measure13, ingredient14 to measure14, ingredient15 to measure15, ingredient16 to measure16,
            ingredient17 to measure17, ingredient18 to measure18, ingredient19 to measure19, ingredient20 to measure20,
        ).mapNotNull { (name, measure) ->
            val trimmedName = name?.trim().orEmpty()
            if (trimmedName.isEmpty()) null else trimmedName to (measure?.trim().orEmpty())
        }
}
